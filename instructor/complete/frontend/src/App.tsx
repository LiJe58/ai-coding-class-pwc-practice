...
function Dashboard({ result, day2, day3, testerUserId, onRun, running, onNavigate }: { result: ApiResult; day2: WorkingPaperResult | null; day3: Day3ReadyResult | null; testerUserId: string; onRun: () => void; running: boolean; onNavigate: (page: Page) => void }) {
  ...
  return <>
    ...
    {day2 && <section className="card day2-overview">
      ...
      <section><div className="section-heading"><div><p className="eyebrow">실행 기록</p><h3>사례별 Agent 실행 이력</h3></div><span className="quiet-badge">{agentRuns.length}건</span></div>{agentRuns.length ? <ol className="timeline">{agentRuns.map((run) => <li key={run.run_id}><span className={`timeline-dot ${run.status === "success" ? "active" : ""}`} /><div><strong>{run.status} · {run.requester_user_id}</strong><div className="agent-review-markdown"><ReactMarkdown>{run.answer ?? run.error_message}</ReactMarkdown></div><small>{run.completed_at} · Tool {run.tool_status} · 모델 {run.model_status}</small></div></li>)}</ol> : <p className="section-note">이 사례에 저장된 Agent 실행 이력이 없습니다.</p>}</section>
    </section>}
    {day3 && <section className="card day3-overview"><div className="section-heading"><div><p className="eyebrow">담당자의 최종 검토 결과</p><h2>Day 3 담당자 검토</h2></div><button className="day3-button" onClick={() => onNavigate("review")}>최종 검토 보기</button></div><div className="day3-metric-grid"><div><span>담당자 검토 완료</span><strong>{day3.reviewed_count} / {day3.total_count}</strong></div><div><span>미검토</span><strong>{day3.pending_count}건</strong></div><div><span>정상</span><strong>{day3.normal_count}건</strong></div><div><span>추가 확인</span><strong>{day3.follow_up_count}건</strong></div><div><span>통제 예외</span><strong>{day3.control_exception_count}건</strong></div></div></section>}
...
function FinalReview({ state, result, paper, error, selectedChangeId, testerUserId, onRetry, onSaved }: { state: Day3RequestState; result: Day3ApiResult | null; paper: WorkingPaperResult | null; error: string; selectedChangeId: string; testerUserId: string; onRetry: () => void; onSaved: () => Promise<void> }) {
  ...
  if (state === "loading") return <div className="state-card" role="status"><span className="spinner" />Day 3 담당자 검토 목록을 불러오고 있습니다.</div>
  ...
  if (state === "empty" || !items.length) return <div className="state-card"><strong>검토 목록 결과가 없습니다.</strong><span>Day 2 지정 표본 12건을 확인해 주세요.</span><button className="secondary-button" onClick={onRetry}>새로고침</button></div>
  ...
  return <>
    ...
    <section className="day3-summary" aria-label="Day 3 담당자 검토 요약"><div><span>검토 완료</span><strong>{result.reviewed_count} / {result.total_count}</strong></div><div><span>미검토</span><strong>{result.pending_count}건</strong></div><div><span>정상</span><strong>{result.normal_count}건</strong></div><div><span>추가 확인</span><strong>{result.follow_up_count}건</strong></div><div><span>통제 예외</span><strong>{result.control_exception_count}건</strong></div></section>
