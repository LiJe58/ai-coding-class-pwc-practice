...
async function requestAgentPreview(changeId: string) {
  const response = await fetch(`/api/day2/agent-preview/${changeId}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ requester_user_id: "U701" }),
  })
...
function Dashboard({ result, day2, day3, onRun, running, onNavigate }: { result: ApiResult; day2: WorkingPaperResult | null; day3: Day3ReadyResult | null; onRun: () => void; running: boolean; onNavigate: (page: Page) => void }) {
  ...
  const runAgentReview = async () => {
    ...
    setAgentError("")
    try {
      setAgentPreview(await requestAgentPreview(selectedAgentChangeId))
  ...
  return <>
    ...
    {day2 && <section className="card day2-overview">
      <div className="section-heading"><div><p className="eyebrow">Agent 실행 결과</p><h2>Day 2 통제조서 초안</h2></div><button className="secondary-button" onClick={() => onNavigate("paper")}>통제조서 보기</button></div>
      <div className="day2-metric-grid"><div><span>고정 표본</span><strong>{day2.summary.sample_count}건</strong></div><div><span>조서 초안</span><strong>{day2.summary.draft_count}건</strong></div><div><span>마지막 Agent 실행</span><strong>{day2.generated_at}</strong></div></div>
...
function WorkingPaper({ state, result, error, selectedChangeId, onRetry, onOpenReview }: { state: Day2RequestState; result: Day2ApiResult | null; error: string; selectedChangeId: string; onRetry: () => void; onOpenReview: (changeId: string) => void }) {
  ...
  if (state === "loading") return <div className="state-card" role="status"><span className="spinner" />Agent 통제조서를 불러오고 있습니다.</div>
  if (state === "not_generated") return <div className="state-card"><strong>통제조서가 아직 생성되지 않았습니다.</strong><span>`/control-test` Skill로 working-paper.json을 생성한 뒤 다시 확인해 주세요.</span><button className="secondary-button" onClick={onRetry}>새로고침</button></div>
  ...
  return <>
    <PageHeader eyebrow="3단계 · 조서 초안" title="통제조서" description="Mock ERP 근거와 Agent 초안을 표본별로 확인합니다. 모든 결론은 사람 검토 전 상태입니다." />
    ...
    <section className="day2-summary" aria-label="Day 2 조서 요약"><div><span>정상 표본</span><strong>{paper.summary.normal_sample_count}건</strong></div><div><span>검토 필요 표본</span><strong>{paper.summary.review_sample_count}건</strong></div><div><span>조서 초안</span><strong>{paper.summary.draft_count}건</strong></div><div><span>사람 검토 대기</span><strong>{paper.summary.sample_count}건</strong></div></section>
...
function FinalReview({ state, result, paper, error, selectedChangeId, onRetry, onSaved }: { state: Day3RequestState; result: Day3ApiResult | null; paper: WorkingPaperResult | null; error: string; selectedChangeId: string; onRetry: () => void; onSaved: () => Promise<void> }) {
  ...
  const submit = async () => {
    ...
    try {
      await saveReview(selected.change_id, { review_action_id: actionId, reviewer_user_id: "U701", conclusion, review_comment: trimmed })
  ...
  const exportCsv = async () => {
    ...
    try {
      const response = await fetch("/api/day3/export.csv?reviewer_user_id=U701")
  ...
  if (state === "blocked" && result?.status === "blocked") return <div className="state-card error-state"><strong>{result.reason === "not_generated" ? "Day 2 통제조서가 아직 생성되지 않았습니다." : "Day 2 working-paper.json이 올바르지 않습니다."}</strong><span>{result.blocked_reason.message}</span><button className="secondary-button" onClick={onRetry}>새로고침 후 복구</button></div>
...
function App() {
  ...
  if (result) {
    if (page === "dashboard") content = <Dashboard result={result} day2={workingPaper} day3={day3Ready} onRun={runControlTest} running={running} onNavigate={navigate} />
