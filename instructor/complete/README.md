...
완료 상태: 규칙 기반 통제부터 Agent 검토자료, 담당자 검토까지 전체 기능, 상태 연결, 완료 조건, 권한 확인, 중복 저장 방지, CSV 내보내기 조건과 적용 범위 템플릿이 포함됩니다. 메인의 `Agent 검토 수행`은 기존 `get_case_evidence` Tool로 선택한 표본의 근거를 읽고 실행 결과를 사례별 이력에 남깁니다.
