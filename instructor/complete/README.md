...
초기 집계는 `12/0/12`, 완료 집계 예시는 `12/12/0/4/1/7`입니다. 12건을 모두 검토하기 전에는 CSV 내보내기가 거부됩니다. 입력 CSV와 Agent 검토자료 파일 `output/day-2/working-paper.json`은 보존하고 SQLite·다운로드 CSV는 실행 중에만 사용하세요. Agent 실행 이력은 SQLite에 저장되지만 사람 결론과 검토자료는 바꾸지 않습니다. 복구는 `python scripts/checkpoint.py reset instructor/complete`를 사용합니다.
