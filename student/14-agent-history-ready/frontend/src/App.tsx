...
type WorkingPaperSample = {
  ...
  vendor_id: string
  vendor_name: string
  selection_reason: "교육용 지정 정상 표본" | "핵심 규칙 위반 전수"
...
async function requestWorkingPaper() {
  ...
  if (!response.ok) throw new Error(`검토자료 API 요청 실패 (${response.status})`)
...
async function requestDay3Reviews() {
  ...
  if (!response.ok) throw new ApiError(`담당자 검토 API 요청 실패 (${response.status})`, response.status)
...
function Dashboard({ result, day2, day3, testerUserId, onRun, running, onNavigate }: { result: ApiResult; day2: WorkingPaperResult | null; day3: Day3ReadyResult | null; testerUserId: string; onRun: () => void; running: boolean; onNavigate: (page: Page) => void }) {
  ...
  return <>
    ...
    {day2 && <section className="card day2-overview">
      <div className="section-heading"><div><p className="eyebrow">Agent 실행 결과</p><h2>Agent 검토자료 초안</h2></div><button className="secondary-button" onClick={() => onNavigate("paper")}>검토자료 보기</button></div>
      <div className="day2-metric-grid"><div><span>지정 표본</span><strong>{day2.summary.sample_count}건</strong></div><div><span>검토자료 초안</span><strong>{day2.summary.draft_count}건</strong></div><div><span>마지막 Agent 실행</span><strong>{day2.generated_at}</strong></div></div>
      <div className="review-meta"><div><span>현재 테스트 사용자</span><strong>{testerUserId} · {testerUserId === "U701" ? "내부통제 검토자" : "지급 업무 담당자"}</strong></div><div><span>Agent 권한</span><strong>{testerUserId === "U701" ? "근거 조회 허용 · 읽기 전용" : "근거 조회 불가"}</strong></div><div><span>실행 범위</span><strong>get_case_evidence 1회 · 최종 검토 결과 변경 불가</strong></div></div>
...
function WorkingPaper({ state, result, error, selectedChangeId, onRetry, onOpenReview }: { state: Day2RequestState; result: Day2ApiResult | null; error: string; selectedChangeId: string; onRetry: () => void; onOpenReview: (changeId: string) => void }) {
  ...
  if (state === "error") return <div className="state-card error-state"><strong>검토자료 API 연결 상태를 확인해 주세요.</strong><span>{error}</span><button className="secondary-button" onClick={onRetry}>다시 연결</button></div>
  if (state === "invalid") {
  ...
  return <>
    <PageHeader eyebrow="3단계 · 검토자료 초안" title="통제 검토자료" description="Mock ERP 근거와 Agent 초안을 표본별로 확인합니다. 모든 결론은 담당자 검토 전 상태입니다." />
    ...
    <section className="day2-summary" aria-label="Agent 검토자료 요약"><div><span>정상 표본</span><strong>{paper.summary.normal_sample_count}건</strong></div><div><span>검토 필요 표본</span><strong>{paper.summary.review_sample_count}건</strong></div><div><span>검토자료 초안</span><strong>{paper.summary.draft_count}건</strong></div><div><span>담당자 검토 대기</span><strong>{paper.summary.sample_count}건</strong></div></section>
    <div className="paper-layout"><section className="card paper-sample-list"><div className="section-heading"><div><p className="eyebrow">지정 표본</p><h2>표본 {samples.length}건</h2></div></div>{samples.map((sample) => <button className={`paper-sample ${selected.change_id === sample.change_id ? "selected" : ""}`} key={sample.change_id} onClick={() => setSelectedId(sample.change_id)}><div><span className="id-cell">{sample.change_id}</span><strong>{sample.vendor_name}</strong><small>{sample.selection_reason}</small></div><StatusBadge status={sample.day1_status} /></button>)}</section>
      <section className="card paper-detail"><div className="detail-title"><div><p className="eyebrow">{selected.sample_id}</p><h2>{selected.vendor_name}</h2><span className="id-cell">{selected.change_id}</span></div><div className="paper-actions"><span className="risk caution">담당자 검토 필요 △</span><button className="day3-button" onClick={() => onOpenReview(selected.change_id)}>담당자 검토로 이동</button></div></div>
        <dl className="paper-meta"><div><dt>사례 ID</dt><dd>{selected.case_id}</dd></div><div><dt>거래처 ID</dt><dd>{selected.vendor_id}</dd></div><div><dt>선정 사유</dt><dd>{selected.selection_reason}</dd></div><div><dt>규칙 판정 상태</dt><dd>{statusText[selected.day1_status]}</dd></div></dl>
        <h3 className="subheading">규칙 판정 결과</h3><div className="paper-rules">{selected.rule_results.map((item) => <div key={item.rule_id}><strong>{item.rule_id} · {item.rule_name}</strong><span className={item.result === "fail" ? "rule-fail" : ""}>{ruleResultText[item.result]}</span><p>{item.detail}</p></div>)}</div>
        ...
        <section className="agent-draft"><div className="draft-banner"><span>Agent 초안</span><strong>담당자의 최종 검토 결과이 아닙니다.</strong></div><div><h3>수행 절차</h3><p>{selected.agent_draft.procedure}</p><h3>확인 사실</h3><ul>{selected.agent_draft.facts.map((fact) => <li key={fact}>{fact}</li>)}</ul><h3>초안 평가</h3><p>{selected.agent_draft.draft_assessment}</p><h3>추가 확인 사항</h3>{selected.agent_draft.additional_follow_up.length ? <ul>{selected.agent_draft.additional_follow_up.map((item) => <li key={item}>{item}</li>)}</ul> : <p>추가 확인 필요 사항이 기록되지 않았습니다.</p>}<div className="reference-row">{selected.agent_draft.citations.map((id) => <span className="reference-chip" key={id}>{id}</span>)}</div></div></section>
      </section>
    </div>
  </>
}
...
function FinalReview({ state, result, paper, error, selectedChangeId, testerUserId, onRetry, onSaved }: { state: Day3RequestState; result: Day3ApiResult | null; paper: WorkingPaperResult | null; error: string; selectedChangeId: string; testerUserId: string; onRetry: () => void; onSaved: () => Promise<void> }) {
  ...
  const submit = async () => {
    ...
    if (!conclusion || !trimmed || trimmed.length > 1000) {
      setSaveState("error"); setSaveMessage(!conclusion ? "담당자의 최종 검토 결과을 선택해 주세요." : !trimmed ? "검토 의견을 입력해 주세요." : "검토 의견은 1,000자 이하로 입력해 주세요."); return
    }
    ...
    try {
      ...
      await onSaved(); setPendingActionId(""); setSaveState("success"); setSaveMessage("담당자의 최종 검토 결과와 감사 이벤트를 저장했습니다.")
    } catch (reason) {
  ...
  const exportCsv = async () => {
    ...
    try {
      ...
      link.href = url; link.download = "human-reviews.csv"; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url)
      setExportState("idle")
    } catch (reason) {
  ...
  if (state === "loading") return <div className="state-card" role="status"><span className="spinner" />담당자 검토 목록을 불러오고 있습니다.</div>
  if (state === "error") return <div className="state-card error-state"><strong>담당자 검토 API에 연결할 수 없습니다.</strong><span>{error}</span><button className="secondary-button" onClick={onRetry}>다시 연결</button></div>
  if (state === "blocked" && result?.status === "blocked") return <div className="state-card error-state"><strong>{result.reason === "not_generated" ? "Agent 검토자료가 아직 생성되지 않았습니다." : "working-paper.json이 올바르지 않습니다."}</strong><span>{result.blocked_reason.message}</span><button className="secondary-button" onClick={onRetry}>새로고침 후 복구</button></div>
  if (state === "empty" || !items.length) return <div className="state-card"><strong>검토 목록 결과가 없습니다.</strong><span>지정 표본 12건을 확인해 주세요.</span><button className="secondary-button" onClick={onRetry}>새로고침</button></div>
  if (!selected || !sample || result?.status !== "ready") return <div className="state-card error-state"><strong>Agent 초안 세부정보를 연결하지 못했습니다.</strong><span>검토자료와 담당자 검토 API를 새로고침해 주세요.</span><button className="secondary-button" onClick={onRetry}>다시 연결</button></div>

  return <>
    <PageHeader eyebrow="4단계 · 담당자 검토" title="최종 검토" description="Agent 초안과 근거를 확인한 사람이 명시적으로 최종 결론과 의견을 저장합니다." action={<button className="day3-button" disabled={!result.export_ready || exportState === "exporting"} onClick={() => void exportCsv()} title={result.export_ready ? "현재 최신 결론 12건을 CSV로 내보냅니다." : `검토 ${result.pending_count}건이 남아 있습니다.`}>{exportState === "exporting" ? "CSV 준비 중…" : "CSV 내보내기"}</button>} />
    <section className="day3-summary" aria-label="담당자 검토 요약"><div><span>검토 완료</span><strong>{result.reviewed_count} / {result.total_count}</strong></div><div><span>미검토</span><strong>{result.pending_count}건</strong></div><div><span>정상</span><strong>{result.normal_count}건</strong></div><div><span>추가 확인</span><strong>{result.follow_up_count}건</strong></div><div><span>통제 예외</span><strong>{result.control_exception_count}건</strong></div></section>
    ...
    <div className="review-layout"><section className="card review-sample-list"><div className="section-heading"><div><p className="eyebrow">지정 표본</p><h2>담당자 검토 {items.length}건</h2></div></div>{items.map((item) => <button className={`review-sample ${selected.change_id === item.change_id ? "selected" : ""}`} key={item.change_id} onClick={() => setSelectedId(item.change_id)}><div><span className="id-cell">{item.change_id}</span><strong>{item.vendor_name}</strong><small>{item.selection_reason}</small></div><span className={`review-status ${item.current_review ? "complete" : "pending"}`}>{item.current_review ? `완료 · ${conclusionText[item.current_review.conclusion]}` : "미검토"}</span></button>)}</section>
      <section className="review-detail"><section className="card"><div className="detail-title"><div><p className="eyebrow">{sample.sample_id}</p><h2>{sample.vendor_name}</h2><span className="id-cell">{sample.change_id}</span></div><span className={`review-status ${selected.current_review ? "complete" : "pending"}`}>{selected.current_review ? "검토 완료" : "미검토"}</span></div>
        <dl className="paper-meta"><div><dt>규칙 판정 상태</dt><dd>{statusText[sample.day1_status]}</dd></div><div><dt>선정 사유</dt><dd>{sample.selection_reason}</dd></div><div><dt>사례 ID</dt><dd>{sample.case_id}</dd></div><div><dt>거래처 ID</dt><dd>{sample.vendor_id}</dd></div></dl>
      ...
      <div className="review-comparison"><section className="card ai-review"><div className="review-label"><span>Agent 검토자료 초안</span><span className="risk caution">최종 결론 아님 △</span></div><h3>수행 절차</h3><p>{sample.agent_draft.procedure}</p><h3 className="subheading">확인 사실</h3><ul>{sample.agent_draft.facts.map((fact) => <li key={fact}>{fact}</li>)}</ul><h3 className="subheading">초안 평가</h3><p>{sample.agent_draft.draft_assessment}</p><h3 className="subheading">추가 확인 사항</h3>{sample.agent_draft.additional_follow_up.length ? <ul>{sample.agent_draft.additional_follow_up.map((item) => <li key={item}>{item}</li>)}</ul> : <p>Agent가 기록한 추가 확인 사항이 없습니다.</p>}<div className="ai-notice">이 Agent 초안은 최종 검토 결과가 아니며 자동으로 선택되지 않습니다.</div></section>
        <section className="card human-review"><div className="review-label"><span>담당자의 최종 검토 결과</span><span className="day3-chip">검토자 {testerUserId}</span></div>{selected.current_review && <div className="current-review"><span>현재 최종 검토 결과</span><strong>{conclusionText[selected.current_review.conclusion]}</strong><p>{selected.current_review.review_comment}</p><small>{selected.current_review.reviewed_at}</small></div>}
...
function App() {
  ...
  const loadDay2 = useCallback(async () => {
    ...
    catch (reason) { setDay2State("error"); setDay2Error(reason instanceof Error ? reason.message : "검토자료 API 연결 오류") }
  }, [])
  const loadDay3 = useCallback(async () => {
    ...
    catch (reason) { setDay3State("error"); setDay3Error(reason instanceof Error ? reason.message : "담당자 검토 API 연결 오류") }
  }, [])
