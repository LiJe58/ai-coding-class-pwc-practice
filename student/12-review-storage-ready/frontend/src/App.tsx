...
export default function App() {
  ...
  if (loading) return <main><h1>통제 테스트 결과</h1><p role="status">데이터를 불러오는 중입니다.</p></main>
  if (error) return <main><h1>통제 테스트 결과</h1><p className="error" role="alert">{error}</p><button onClick={load}>다시 시도</button></main>
  const detail = data?.exceptions.find((item) => item.change_id === selected)
  return <main><p className="eyebrow">Agent 검토자료 준비 완료</p><h1>거래처 변경 통제</h1><section className="metrics">{data && Object.entries(data.summary).map(([key, value]) => <div key={key}><strong>{labels[key]}</strong><span>{value}</span></div>)}</section>{paper && <section><h2>Agent 통제 검토자료</h2><p>표본 {paper.summary.sample_count} · 정상 {paper.summary.normal_sample_count} · 검토 필요 {paper.summary.review_sample_count} · 초안 {paper.summary.draft_count}</p>{paper.samples.map((sample) => <article key={sample.change_id}><strong>{sample.change_id}</strong><span>담당자 검토 필요</span><p>{sample.agent_draft.draft_assessment}</p></article>)}</section>}<div className="layout"><section><h2>예외 목록</h2>{data?.exceptions.map((item) => <button className={selected === item.change_id ? "selected" : ""} key={item.change_id} onClick={() => setSelected(item.change_id)}>{item.change_id} · {item.vendor_name}</button>)}</section><section><h2>{detail?.change_id} 상세</h2><p>{detail?.reason}</p>{detail?.rules.map((rule) => <div key={rule.rule_id}><strong>{rule.rule_id} · {rule.rule_name}</strong><span>{rule.result}</span><small>{rule.detail}</small></div>)}<p>승인 {detail?.approval_ids.join(", ") || "없음"} · 증빙 {detail?.evidence_ids.join(", ")}</p></section></div></main>
}
