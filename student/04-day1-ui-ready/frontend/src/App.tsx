...
function Workflow() {
  ...
  return <section className="card"><div className="section-heading"><div><p className="eyebrow">학습 진행</p><h2>통제 테스트 흐름</h2></div><span className="quiet-badge">검증 대기</span></div><ol className="workflow">{labels.map((label, index) => <li className={index < current ? "done" : index === current ? "current" : "pending"} key={label}><span>{index < current ? "✓" : index + 1}</span><div><strong>{label}</strong><small>{index < current ? "확인 완료" : index === current ? "이번 단계" : "다음 단계에서 연결"}</small></div></li>)}</ol></section>
}
