...
이후 `python scripts/checkpoint.py reset <체크포인트>`는 `.venv`와 `frontend/node_modules`를 보존하고, 공통 `assets/scenario`를 작업공간에 함께 복사합니다. 각 상태에서 `npm run check`, `npm run start:backend`, `npm run dev:frontend`를 사용합니다.
